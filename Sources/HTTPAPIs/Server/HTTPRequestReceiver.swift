...
/// A protocol for receiving an HTTP request body and trailing fields.
///
/// ``HTTPRequestReceiver`` is used on the server side to incrementally consume
/// the body bytes of an incoming HTTP request and capture any trailing
/// ``HTTPFields`` that follow the body.
@available(macOS 26.2, iOS 26.2, watchOS 26.2, tvOS 26.2, visionOS 26.2, *)
public protocol HTTPRequestReceiver<Reader>: ~Copyable, ~Escapable {
    /// The asynchronous reader type that supplies request body bytes.
    associatedtype Reader: AsyncReader, ~Copyable, ~Escapable
    where Reader.ReadElement == UInt8

    /// Receives the request body bytes and concludes with optional trailing fields.
    ///
...
@available(macOS 26.2, iOS 26.2, watchOS 26.2, tvOS 26.2, visionOS 26.2, *)
extension HTTPRequestReceiver where Self: ~Copyable, Reader: ~Copyable {
    /// Reads body bytes into the supplied buffer until either its free capacity is
    /// exhausted or the request stream ends, and returns any trailing fields.
    ///
    ...
    /// Collects up to `limit` bytes from the request body and processes them via `body`.
    ///
