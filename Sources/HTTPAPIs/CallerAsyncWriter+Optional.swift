...
extension CallerAsyncWriter where Self: ~Copyable, Self: ~Escapable, WriteElement: ~Copyable {
    /// Concludes the writer with no remaining buffer and no payload, when the
    /// ``FinalElement`` is `Optional`. Equivalent to
    /// ``finish(buffer:finalElement:)`` with an empty buffer and `nil`.
    ...
    /// Concludes the writer with the supplied buffer and no payload, when the
    /// ``FinalElement`` is `Optional`. Equivalent to
    /// ``finish(buffer:finalElement:)`` with `finalElement: nil`.
