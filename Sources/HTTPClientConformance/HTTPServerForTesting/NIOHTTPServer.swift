...
public struct NIOHTTPServer: HTTPServer {
    public typealias RequestReceiver = NIOHTTPRequestReceiver
    ...
    /// )
    /// ```
    public func serve(handler: some HTTPServerRequestHandler<RequestReceiver, ResponseSender>) async throws {
        defer {
    ...
    func handleRequestChannel(
        ...
        handler: some HTTPServerRequestHandler<RequestReceiver, ResponseSender>
    ) async throws {
        do {
            try await channel
                .executeThenClose { inbound, outbound in
                    ...
                    let readerState = NIOHTTPRequestReceiver.ReaderState()
                    ...
                    do {
                        try await handler.handle(
                            request: httpRequest,
                            requestContext: HTTPRequestContext(),
                            requestReceiver: NIOHTTPRequestReceiver(
                                iterator: iterator,
