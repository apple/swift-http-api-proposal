    ...
    func serve() {
        ...
        Task {
            try await server.serve { request, requestContext, requestBodyAndTrailers, responseSender in
                ...
                case "/gzip":
                    // OK
                    let response_headers = HTTPFields([HTTPField(name: .init("Content-Encoding")!, value: "gzip")])
                ...
                case "/brotli":
                    // OK
                    let response_headers = HTTPFields([HTTPField(name: .init("Content-Encoding")!, value: "br")])
                    let writer = try await responseSender.send(HTTPResponse(status: .ok, headerFields: response_headers))
                ...
                case "/301":
                    // Redirect to /get
                    let writer = try await responseSender.send(HTTPResponse(status: .movedPermanently, headerFields: HTTPFields([HTTPField(name: .init("Location")!, value: "/get")])))
                    try await writer.writeAndConclude("".utf8.span, finalElement: nil)
                case "/308":
                    // Redirect to /get
                    let writer = try await responseSender.send(HTTPResponse(status: .permanentRedirect, headerFields: HTTPFields([HTTPField(name: .init("Location")!, value: "/get")])))
                    try await writer.writeAndConclude("".utf8.span, finalElement: nil)
                case "/404":
